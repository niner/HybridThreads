...
The scheduler is the place where most of the green thread logic is implemented. It consists of two parts: functions using the prefix \textit{Parrot\_cx} which are located in \textit{src/scheduler.c} and the \textit{Scheduler} PMC type containing the scheduler's data and public interface.

The process of executing and preempting tasks is pictured in figure~\ref{fig:controlflow} and described in detail in the rest of this section.

\begin{figure}
\centering
\begin{tikzpicture}[node distance = 1cm, auto]
\node[endpoint](start){start};
\node[block, below of=start, node distance=1.1cm](outerrl){Parrot\_cx\_outer\_runloop};
\node[decision, below of=outerrl] (moretasks) {tasks available?};
\node[block, below of=moretasks, node distance=2cm](nexttask){Parrot\_cx\_next\_task};
\node[endpoint, right of=moretasks, node distance = 13.8em](end){end};
\node[block, below of=nexttask](runloop){runloop};
\node[decision, below of=runloop, node distance=1.7cm] (branch) {branch?};
\node[block, below of=branch, node distance=2cm](checksched){Parrot\_cx\_check\_scheduler};
\node[block, below of=checksched](runsched){Parrot\_cx\_run\_scheduler};
\node[block, right of=runsched, node distance=13.8em](checkquant){Parot\_cx\_check\_quantum};
\node[decision, below of=runsched] (taskdone) {Quantum done?};
\node[block, below of=taskdone, node distance=2cm](preempt){Parrot\_cx\_preempt\_task};
\node[block, right of=preempt, node distance=13.8em](stop){Parrot\_cx\_stop\_task};
\path[line](start) -- (outerrl);
\path[line](outerrl) -- (moretasks);
\path[line](moretasks.south) to node[right] {yes} (nexttask.north);
\path[line](moretasks.east) to  node[below] {no} (end.west);
\path[line](nexttask) -- (runloop);
\path[line](runloop) -- (branch);
\path[line](branch.south) to node[right] {yes} (checksched.north);
\path[line](branch.west) to [out=180,in=230] node[right] {no} (runloop.west);
\path[line](checksched) -- (runsched);
\path[line](runsched) -- (checkquant);
\path[line](checkquant)[yshift=-0.3cm] -- (runsched)[yshift=-0.3cm];
\path[line](runsched) -- (taskdone);
\path[line](taskdone.south) to node[right] {yes} (preempt.north);
\path[line](taskdone.west) to [out=150,in=230] node[right] {no} (runloop.west);
\path[line](preempt) -- (stop);
\path[line](stop)[yshift=-0.3cm] -- (preempt)[yshift=-0.3cm];
\draw[->] (preempt.west) to [out=110,in=250] (outerrl.west);
\end{tikzpicture}
\caption{Control flow of task execution and preemption.}
...
\item \textit{PMC *partner}: copy of this task in another thread.
\end{itemize}
...
\chapter{\textit{matrix\_part.winxed}}
