...
The process of executing and preempting tasks is pictured in figure~\ref{controlflow} and described in detail in the rest of this section.

\begin{figure}
\centering
\begin{tikzpicture}[node distance = 1cm, auto]
\node[block](outerrl){Parrot\_cx\_outer\_runloop};
\node[block, below of=outerrl](nexttask){Parrot\_cx\_next\_task};
\node[block, below of=nexttask](runloop){runloop};
\node[decision, below of=runloop] (branch) {branch?};
...
\path[line](taskdone.west) to [out=180,in=220] node[right] {no} (runloop.west);
\path[line](preempt) -- (stop);
\draw[->] (preempt.west) to [out=120,in=240] (outerrl.west);
\end{tikzpicture}
