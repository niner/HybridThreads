...
Proxies are the arbiters between threads. They are the only means for a thread to access another thread's data and are implemented by the \textit{Proxy} PMC type.

\subsection{Writing shared data (2)}

As described in chapter~\ref{cha:design} to write to shared data, a thread creates a small task and schedules it on the data owning interpreter. Such a task could look like:
%
\begin{GenericCode}
