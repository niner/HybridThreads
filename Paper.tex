...
\textit{push\_pmc} and \textit{unshift\_pmc} are used to add a task at the end respectively the beginning of the task\_queue. \textit{shift\_pmc} is used to fetch and remove a task from the tip of the task\_queue. \textit{get\_integer} returns the number of tasks in the task\_queue. These four methods use the task\_queue\_lock to make accessing the task\_queue thread safe.
...
The textit{active\_tasks} returns an array containing all tasks which have been run at least once and are not yet finished, e.g. they are currently being executed or are preempted.
